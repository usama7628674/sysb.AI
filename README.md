...
```
It will give us requested static subdomain `https://my-awesome-subdomain.us.sysb.ai`. Alternatively, we can also use the command line flags of ssh to get it done, like below
```
...
```
Then we need to have below entry in `~/.ssh/config` before executing `ssh -R 0:localhost:8080 demo@sysb.ai`
```
Host sysb.ai
